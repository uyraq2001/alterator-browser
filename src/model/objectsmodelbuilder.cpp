...
ObjectsModelBuilder::ObjectsModelBuilder(QString serviceName,
    ...
    , m_dbusFindInterfaces(std::move(findInterfaceList))
...
std::unique_ptr<Model> ObjectsModelBuilder::buildModel()
{
    ...
    if (m_dbusFindInterfaces.isEmpty())
    {
        qWarning() << "List of interfaces is empty. Can't find any objects!";
    }

    std::unique_ptr<Object> objects{};

    for (QString &currentIface : m_dbusFindInterfaces)
    {
        QStringList pathsOfObjects = getListOfObjects(currentIface);

        if (pathsOfObjects.empty())
        {
            qWarning() << "Can't find any objects with interface: " << currentIface;
            continue;
        }

        std::vector<std::unique_ptr<Object>> parsedObjects = parseObjects(pathsOfObjects, currentIface);

        //TO DO add parsed objects to model
    }
...
QStringList ObjectsModelBuilder::getListOfObjects(QString iface)
{
    QDBusInterface managerIface(m_dbusServiceName, m_dbusPath, m_managerInterface, m_dbusConnection);

    if (!managerIface.isValid())
    {
        qCritical() << "Can't access alterator manager interface: " << iface;
        return {};
    }
    ...
    if (!reply.isValid())
    {
        qCritical() << "Can't get reply from alterator manager: " << iface << " interface: " << reply.error().message();
        return {};
    }
...
std::vector<std::unique_ptr<Object>> ObjectsModelBuilder::parseObjects(QStringList &pathsList, QString &iface)
{
    std::vector<std::unique_ptr<Object>> acObjects;

    for (QString &currentPath : pathsList)
    {
        QDBusInterface currentIface(m_dbusServiceName, currentPath, iface, m_dbusConnection);

        if (!currentIface.isValid())
        {
            qWarning() << "Object " << currentPath << " does not provide interface " << iface;
            continue;
        }

        QString currentObjectInfo = getObjectInfo(currentIface);

        if (currentObjectInfo.isEmpty())
        {
            qWarning() << "Can't get info of object" << currentPath << "in interface" << iface;
            continue;
        }
        ...
        if (!objectBuilder)
        {
            qWarning() << "Bad info format in object" << currentPath << "in interface" << iface;
            continue;
        }

        std::unique_ptr<Object> newObject = std::move(objectBuilder->buildObject(&infoParsingResult));
